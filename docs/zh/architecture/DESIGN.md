...
simulators / adapters / 外部组件
    → gf_ara API
    → middleware (可裁剪)  ← EM 用 OSAL Process，不直接 fork/exec
    → bindings (iceoryx | someip | dds | cross_domain_ipc)
    → OSAL (posix + arch: arm | mips | riscv)
         clock / thread / process (Spawn·Wait·Terminate·Kill)
    → HAL (boards/)
```

换 SoC：**只动 OSAL arch + HAL + profile**。禁止业务 `#ifdef SOC_X`。  
进程生命周期：Linux 走 `middleware/osal/src/posix/process.cpp`；QNX/其他 OS 换 backend，**`EmDaemon` 源码不动**。
...
配置：`cfg/gf_ara_cfg/diag.yaml` / `ucm.yaml`（gf-config）。操作：GMT OTA 页（不写配置）。说明：[DOIP_OTA.md](../../operations/DOIP_OTA.md)。
