...
3. 否则仓库相对路径 `projects/afc/generated/camera_contract.json`（若存在）  
4. 都没有 / 字段不全 → **硬退出**（不发明数字）

```bash
# 例：Windows/另一台机跑场景时显式对齐
export GF_CAMERA_CONTRACT=/path/to/afc/generated/camera_contract.json
# 或
export GF_PROJECT_DIR=/path/to/afc
