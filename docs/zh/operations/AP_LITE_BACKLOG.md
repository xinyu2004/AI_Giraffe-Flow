...
| 项 | 说明 |
|----|------|
| see-cone ambient pop-in | 准则已写；`_traffic.py` spawn band 仍可锥内刷 — 下波改 |
| Gateway 触发可配 / 映射表 | 见 `gateway-trigger-flexibility` 规则与 `gateway_boundary.md` 遗留债 |
| Truth 非几何质量字段 | `backlog_truth_quality.md` |
| 加速度手感 / 起点漂移 / 遇不到红灯 | 体验项，先不管 |
| `bev_paint/` 物理抽出 | Host 入口稳定后再搬目录 |
| Creep `reg_stop_creep_m` | **不恢复**：近线刹停合同 = 规划 `hold≈1 m` 内 v→0；不以 creep 标定补洞 |
