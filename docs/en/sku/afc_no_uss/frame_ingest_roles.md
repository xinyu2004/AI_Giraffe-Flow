# frame_ingest / tip / carla_scenarios roles
...
- **Tip / frame source**: freeze SOP default `isp` (into `frame_ingest_config.hpp`); SIL overrides with **`GF_FRAME_SOURCE`**: `isp` / `carla` / `replay` / `colorbar` (legacy `synth`) / `none`. Orthogonal to `GF_INJECT_MODE` (ego/SOA inject). Compat aliases: `GF_TIP_SOURCE` / `GF_ACTIVE_SOURCE`.
- **ego_source**: `gateway` / `carla` / `inject` — same freeze; binaries read constexpr. **Do not** grep the hpp from `run_sil`.
- **`carla_scenarios/`**: independent Client A scenario machine — **zero** compose / stamp / runtime contract. Do not call tip-source selection a "scenario".
- **compile / run do not compose**: after tip edits, **Verify** in gf-config; `compile_sil` / `run_sil` only build/run existing `generated/`.
...
No copy into `carla_scenarios/` — re-compose after tip edits. SIL tip writer (`carla_bridge`) prefers ingest `GF_CAMERA_MOUNT_*` from hpp; falls back to the same `camera_contract.json`.

## Resident `gf_frame_ingest` (C++)

`run_sil` **always** starts `build-sil/runtime/bin/gf_frame_ingest`:

- Freeze `kBridgeEnabled=false` or `kActiveSource=none` → **exit 0 immediately**
- Else Create GfChannel, spawn Python module (`--module-only`)
- Shared lib at `runtime/lib/libgf_gf_channel.so` (RPATH / `LD_LIBRARY_PATH`); **no `GF_TIP_LIB`**

| Role | Owns | Does not |
|------|------|----------|
| **gf_frame_ingest (C++)** | Read hpp; Create slots; spawn module | World layout / ACC plot |
| **carla module** | Camera→YUV, ego, apply cmd | Spawn hero |
| **GfChannel** | shm image slots | ARA events |
| **FCM** | Open+Latest | Create |
| **carla_scenarios** | Client A place+IC | Tip / compose |
...
- No `GF_TIP_LIB`; no script grep of `frame_ingest_config.hpp`
