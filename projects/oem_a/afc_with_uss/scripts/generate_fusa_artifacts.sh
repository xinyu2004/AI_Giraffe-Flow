#!/usr/bin/env bash
# Generate FuSa evidence / artifacts for this SKU → fusa/packs/oem_a_afc_with_uss/.
...
if [[ -d "${LOG_DIR}" ]]; then
  for f in gateway.log fcm.log uss.log planning.log giraffe_modules.log; do
...
{
  echo "# oem_a / afc_with_uss FuSa Evidence Manifest"
  echo
  echo "Generated: ${TS}  ·  git: ${GIT_REV}"
  if [[ "${RELEASE}" == "1" ]]; then
  ...
  for rel in compose/gf.sor.json lineage/signal_lineage_report.yaml mcap/session.mcap \
             ...
             smoke/sil_verify.txt smoke/observability.txt runs/cases_latest.log \
  ...
  echo "GF_FUSA_PACK_RELEASE=1 bash projects/oem_a/afc_with_uss/scripts/generate_fusa_artifacts.sh"
