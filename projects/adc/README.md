...
域控制器项目：前视 + 环视 + 泊车/行车规划 + MCU（接口树可含 USS 类型历史文件；产品 AFC 路径无 USS）。
...
gf-config projects/adc/project.yaml

# CI / 无 GUI
python -m gf_codegen.compose --project projects/adc/project.yaml
gf-codegen lint projects/adc/gf.sor.json
gf-codegen generate projects/adc/gf.sor.json --out projects/adc/generated/

# lineage：projects/adc/reports/signal_lineage_report.yaml
```

iceoryx 双进程 SIL 演示仍以 [`afc`](../afc/) 为准（拓扑更小）：
