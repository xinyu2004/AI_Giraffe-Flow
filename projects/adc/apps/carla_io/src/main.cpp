    ...
    while (!g_stop) {
      ...
      switch (hdr.msg_type) {
        ...
        case GF_COSIM_MSG_SURROUND_WORLD:
          ...
          break;
        case GF_COSIM_MSG_MODE_HINT:
          if (hint && payload.size() >= sizeof(GfModeHintPod)) {
            GfModeHintPod pod{};
            std::memcpy(&pod, payload.data(), sizeof(pod));
            if (pod.magic == GF_CH_MODE_HINT_MAGIC &&
                pod.version == GF_CH_MODE_HINT_VERSION) {
              (void)gf_channel_publish(hint, &pod, sizeof(pod), hdr.timestamp_ns, hdr.seq);
            }
          }
          break;
