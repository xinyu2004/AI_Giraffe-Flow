...
FrameSource::FrameSource(FrameSourceKind kind) : kind_(kind) {
  ...
  if (kind_ == FrameSourceKind::File || kind_ == FrameSourceKind::CarlaFile) {
    // Explicit path only — no silent runtime_ipc invention.
    plane_path_ = EnvOr("GF_CARLA_FRAME_PATH", "");
#if defined(GF_FCM_HAS_FRAME_INGEST)
    if (plane_path_.empty() && gf_gen::frame_ingest::kFramePath[0] != '\0') {
      plane_path_ = gf_gen::frame_ingest::kFramePath;
    }
#endif
    if (plane_path_.empty()) {
      std::cerr << "[ERROR] perception.fcm: file transport needs GF_CARLA_FRAME_PATH "
                   "(or req.frame_ingest.paths.frame)\n";
    } else {
