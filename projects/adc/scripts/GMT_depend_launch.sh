...
# Live tap fans to GMT + Foxglove only; JSONL recording is explicit via GMT GUI Record.
#
...
if not kill_stale:
    ...
    print(f"{tag}   GF_SIL_KILL_STALE=1 bash projects/adc/scripts/run_sil.sh …", flush=True)
    raise SystemExit(1)
...
if [[ "${INJECT_ON}" == "1" ]]; then
  ...
  if [[ "${DRIVE_MODE}" == "playhead" || "${DRIVE_MODE}" == "controlled" || "${DRIVE_MODE}" == "wait" ]]; then
    ...
    echo "${TAG} inject stopped (apps=${RUN_APPS}); module logs via DLT"
    exit 0
  fi
  ...
  echo "${TAG} inject finished (apps=${RUN_APPS}); module logs via DLT"
  exit 0
fi

# EM owns gf_frame_ingest on the product path. Shell-start only when inject stopped EM.
# Camera module selected by GF_FRAME_SOURCE (isp|carla|replay|colorbar|none); inherited from run_sil.
CARLA_BRIDGE_PID=""
BRIDGE_TAIL_PID=""
...
host_info "apps under EM — logs via DLT (GMT Record for obs JSONL)"
echo "${TAG} [EM] apps managed by EM pid=${EM_PID} (no direct spawn)"

if [[ "${LIVE_ON}" != "1" ]]; then
  echo "${TAG} live_tap off — EM only; logs via DLT"
