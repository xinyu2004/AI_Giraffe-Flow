...
#   bash projects/adc/scripts/generate_fusa_artifacts.sh
...
if [[ "${GF_FUSA_PACK_RUN_SMOKE:-0}" == "1" ]]; then
  echo "${TAG} smoke_sil ..."
  bash "${ROOT}/projects/adc/scripts/verify/smoke_sil.sh" | tee "${PACK}/smoke/sil.txt"
  echo "${TAG} smoke_sil_observability ..."
  GF_SKIP_COMPILE=1 bash "${ROOT}/projects/adc/scripts/verify/smoke_sil_observability.sh" | tee "${PACK}/smoke/observability.txt"
fi
...

{
  ...
  echo "GF_FUSA_PACK_RELEASE=1 bash projects/adc/scripts/generate_fusa_artifacts.sh"
  echo "# local optional smokes into pack:"
  echo "GF_FUSA_PACK_UPDATE_GOLDEN=1 GF_FUSA_PACK_RUN_SMOKE=1 \\"
  echo "  bash projects/adc/scripts/generate_fusa_artifacts.sh"
  echo '```'
  echo
  echo "## Related"
  echo
  ...
  echo "- Golden: \`projects/adc/golden/gf.sor.json\` (gitignored by default)"
} > "${PACK}/MANIFEST.md"
