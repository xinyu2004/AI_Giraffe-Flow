...
#   GF_INJECT_MODE=playhead bash projects/adc/scripts/run_sil.sh
#   # continuous (board-side file):
#   GF_INJECT_SESSION=projects/adc/build-sil/observability/session.jsonl \
#     bash projects/adc/scripts/run_sil.sh
...
#                    次要覆盖：GF_ACTIVE_SOURCE；synth → colorbar
...
_gf_load_carla_env_file() {
  ...
  while IFS= read -r line || [[ -n "${line}" ]]; do
    ...
    case "${key}" in
      CARLA_HOST|CARLA_PORT|GF_CARLA_WAIT_S|GF_CARLA_CONNECT_TIMEOUT_S|GF_CARLA_PYTHON|GF_CARLA_FRAME_PATH|GF_PIXEL_FORMAT|GF_FRAME_SOURCE|GF_ACTIVE_SOURCE|GF_EGO_SOURCE|GF_REPLAY_DIR|GF_VEHICLE_STATE_SLOT|GF_VEHICLE_CMD_SLOT|GF_FAKE_PERC_SLOT|GF_COSIM_HOST|GF_COSIM_PORT|GF_CTRL_TIP_PORT|GF_CTRL_TIP_HOST)
...
# Secondary: GF_ACTIVE_SOURCE → GF_FRAME_SOURCE if primary unset
if [[ -z "${GF_FRAME_SOURCE:-}" && -n "${GF_ACTIVE_SOURCE:-}" ]]; then
...
if [[ "${GF_FRAME_SOURCE}" == "isp" ]]; then
  echo "${TAG}   WARN: isp on SIL = stub (shm open, no pixels). Foxglove Image stays blank."
  echo "${TAG}         Use GF_FRAME_SOURCE=colorbar (no CARLA) or carla|replay for real frames."
fi
...
for bin in "${NEED_BINS[@]}"; do
  if [[ ! -x "${bin}" ]]; then
    ...
    echo "Build first: bash projects/adc/scripts/compile_sil.sh" >&2
    if [[ "${bin}" == "${TAP}" ]]; then
...
# Module logs → console + DLT. No --log-dir / no build-sil/logs.
GF_DLT_LOG="${BUILD}/middleware/log/gf_dlt_log"
# Scratch for GMT_depend sidecars only (inject fifo); not EM app logs.
GF_GMT_TMP="$(mktemp -d /tmp/gf-gmt-XXXXXX)"
export LOG_DIR="${GF_GMT_TMP}"

# Host-phase Info → stdout; optional gf_dlt_log → DLT when daemon is up.
...
for _i in $(seq 1 50); do
  if ! kill -0 "${EM_PID}" 2>/dev/null; then
    host_info "EM exited early"
    echo "${TAG} EM failed (see console/DLT)" >&2
    exit 1
  fi
  if pgrep -f '[g]f_planning_driving' >/dev/null 2>&1; then
    _em_ready=1
    break
  fi
  sleep 0.2
