"""ctypes client for libgf_channel.so (Open/latest only — Foxglove tip reader)."""
...
def _find_lib() -> Path:
    ...
    for part in (os.environ.get("LD_LIBRARY_PATH") or "").split(":"):
        if part:
            ...
            candidates.append(Path(part) / "libgf_tip_channel.so")
...
class GfChannelReader:
    """Read-only Open + latest for a tip/channel slot."""
    ...
    def _refresh_info(self) -> None:
        ...
        need = int(plane_bytes.value) or (self._w * self._h * 3)
    ...
    def latest(self) -> Optional[tuple[bytes, int, int]]:
        ...
        self._h = int(h.value) or self._h
        name = self._lib.gf_channel_format_name(fmt.value)
        if name:
