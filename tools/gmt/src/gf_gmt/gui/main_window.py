...
from gf_gmt.gui.inject_client import DEFAULT_INJECT_PORT, InjectCtrlClient
...
class GmtMainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self._btn_live_connect.setToolTip(
            "连 live_tap 旁路（ws:8766）；回灌时可同时开，默认只记下游"
        )
        ...
        self._proj_banner = QLabel(
            "⚠ 请先「加载项目…」选择 project.yaml（回灌已禁用；Live 仍可仅记盘）"
        )
    ...
    def _refresh_conn_bar_ui(self) -> None:
        ...
        red = "color:#c62828; font-weight:700; min-width: 5em;"
        idle_s = "color:#555; min-width: 5em;"

        if live_on:
            if self._follow_latest.isChecked():
                self._live_state.setText("已连接·跟随")
            else:
                self._live_state.setText("已连接·只记盘")
        ...
        if inj_on:
            if self._inject_last_ok is True:
                self._inject_state.setText("已灌")
                self._inject_state.setStyleSheet(green)
            elif self._inject_last_ok is False:
                self._inject_state.setText("跳过")
    ...
    def _on_follow_latest_toggled(self, on: bool) -> None:
        if on and self._inject_panel.wants_playhead_sync():
            ...
            self.statusBar().showMessage(
                "回灌「跟 playhead 灌」已开 → Live 跟随已禁用（只记盘仍可连 Live）",
                5000,
            )
            return
        self._stick_tail = on
    ...
    def start_follow(self, path: Path | None = None) -> None:
        ...
        mode = "跟随最新" if self._stick_tail else "只记盘"
    ...
    def _connect_live(self) -> None:
        """Primary UX: connect to external live bridge (Foxglove-style)."""
        if self._live_active:
            return
        if self._inject_panel.wants_playhead_sync():
            self._force_live_follow_off(
                reason="已开回灌 playhead → Live 以只记盘方式连接（不跟随最新）"
            )
        if self._project_dir is None:
            ...
            if self._project_dir is None:
                self.statusBar().showMessage(
                    "Live 无项目：仅记盘（动画 DAG 为空）",
                    5000,
                )

        host = self._host_edit.text().strip() or "127.0.0.1"
        port = int(self._port_spin.value())
        live = self._default_live_session()
        live.parent.mkdir(parents=True, exist_ok=True)
        live.write_text("", encoding="utf-8")
        self._close_live_log()
        self._live_log_fp = live.open("a", encoding="utf-8")
        ...
        self.statusBar().showMessage(
            f"Live 已连接 ws://{host}:{port} → {live}（{mode}）",
            8000,
        )
    ...
    def _connect_inject(self, host: str, port: int) -> None:
        ...
        if self._model.empty:
            QMessageBox.warning(
                self,
                "回灌",
                "请先打开与 inject 相同的 session JSONL，\n"
                "以便 playhead 索引与 inject 对齐。",
            )
        ...
        if self._inject_panel.wants_playhead_sync():
            self._force_live_follow_off(
                reason="回灌已连且跟 playhead → Live 跟随已关（可另连 Live 只记盘旁观）"
            )
        self._inject_timer.start()
        self.statusBar().showMessage(
            f"Inject 已连接 tcp://{host}:{port}",
            6000,
        )
    ...
    def _on_inject_tick(self) -> None:
        if self._inject is None or not self._inject.connected:
            return
        for msg in self._inject.poll_messages():
            if msg.get("op") == "published":
                ...
                self._refresh_conn_bar_ui()
                self.statusBar().showMessage(f"回灌失败：{msg.get('msg')}", 5000)
    ...
    def _apply_inject_published(self, msg: dict[str, Any]) -> None:
        ...
        if index_i >= 0:
            self._inject_panel.record_result(
                ...
                injected=ok,
                topic=topic,
                reason="" if ok else "非可灌 / injected=false",
                t_ns=t_i,
            )
        if ok:
            ...
            self._inject_panel.set_detail(
                f"跳过 #{idx} {topic}（非可灌 / injected=false）"
            )
            self.statusBar().showMessage(
                f"回灌跳过：#{idx} {topic} 不在白名单，未 Send",
                4000,
            )
    ...
    def _seek_index(self, index: int) -> None:
        ...
        if self._follow.isChecked() or self._live_active:
            live = " 跟随" if self._follow_latest.isChecked() else " 只记盘"
