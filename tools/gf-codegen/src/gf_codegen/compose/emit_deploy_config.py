...
from gf_codegen.compose.fg_schema import normalize_active_in, normalize_function_groups
...
def emit_deploy_config_hpp(
    ...
    *,
    ...
    function_groups: list[dict[str, Any]] | None = None,
    out_path: Path,
) -> None:
    deps_by_name = {
    ...
    for fg in fgs:
        ident = _ident(fg["id"])
        states = list(fg.get("states") or [])
        if states:
            lines.append(f"inline constexpr const char* kFgStates_{ident}[] = {{")
            lines.append("  " + ", ".join(_c_str(s) for s in states) + ",")
            lines.append("};")
            lines.append("")

    lines.append("inline constexpr FunctionGroupEntry kFunctionGroups[] = {")
    for fg in fgs:
        ident = _ident(fg["id"])
        states = list(fg.get("states") or [])
        kind_enum = "FgKind::kMode" if fg.get("kind") == "mode" else "FgKind::kMachine"
        states_ptr = f"kFgStates_{ident}" if states else "nullptr"
        lines.append(
            "  {"
            f"{_c_str(fg['id'])}, {kind_enum}, {_c_str(str(fg.get('initial') or ''))}, "
            f"{states_ptr}, {len(states)}u"
            "},"
        )
    lines.append("};")
    lines.append(
        "inline constexpr std::size_t kFunctionGroupCount = "
        "sizeof(kFunctionGroups) / sizeof(kFunctionGroups[0]);"
    )
    lines.append("")
    ...
    for p in launch_procs:
        ...
        ep = exec_by_name.get(name) or {}
        fg = str(ep.get("function_group") or p.get("function_group") or "MachineFG").strip()
        active = normalize_active_in(ep.get("active_in") or p.get("active_in"))
        ...
        entry_lines.append(
            ...
            f"{_c_str(fg)}, {active_ptr}, {n_active}u"
            "},"
        )
...
def emit_deploy_config(
    ...
    *,
    ...
    emit_deploy_config_hpp(
        ...
        function_groups=list(tables["exec"].get("function_groups") or []),
        out_path=hpp,
    )
    return {
        "hpp": str(hpp),
