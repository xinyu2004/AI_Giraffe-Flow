...
def test_compose_afc_writes_gf_ara_cfg_manifest(repo_root: Path, tmp_path: Path) -> None:
    ...
    assert "ucm" in pm  # listed in afc runtime_modules
...
def test_bad_exec_process_fails_compose(repo_root: Path, tmp_path: Path) -> None:
    ...
    shutil.copytree(
        src,
        dst,
        ignore=shutil.ignore_patterns("generated", "reports", "gf.sor.json", "golden", "build-sil", "build-hil", "runtime"),
    )
    # rewrite project paths to be self-contained (already relative)
    exec_path = dst / "cfg" / "gf_ara_cfg" / "exec.yaml"
    ...
    rc = compose_project(dst / "giraffe.yaml", repo_root=repo_root, out=out)
    assert rc == 1
...
def test_adc_loads_gf_ara_cfg(repo_root: Path, tmp_path: Path) -> None:
    """adc giraffe.yaml indexes gf_ara_cfg → compose writes gf_ara_cfg_manifest."""
