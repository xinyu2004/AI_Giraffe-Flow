...
def test_live_tap_adds_app_on_debug() -> None:
    ...
    assert apps.count(FOX_APP) == 1
    assert apps.count(INJECT_APP) == 1
    assert TAP_APP in apps and FOX_APP in apps and INJECT_APP in apps
...
def test_compose_afc_writes_observability(repo_root: Path, tmp_path: Path) -> None:
    ...
    assert "gmt_board/iox_obs_tap" in cmake
    assert "gmt_board/iox_obs_foxglove" in cmake
    assert "gmt_board/iox_obs_inject" in cmake
...
def test_generate_writes_obs_tap_main(repo_root: Path, tmp_path: Path) -> None:
    ...
    assert "Perception_DSTSR_Out" in txt
    assert "m_tsr_num" in txt
    assert "Perception_STATIC_OBJ_Out" in txt
    assert "m_Static_OBJ_Count" in txt
    fox = out / "src/obs_foxglove_main.cpp"
    assert fox.is_file()
    ftxt = fox.read_text(encoding="utf-8")
    assert "gf_foxglove::WsHub" in ftxt
    assert "gf-foxglove-ws" in ftxt
    assert "/gf/driving/bev/compressed" in ftxt
