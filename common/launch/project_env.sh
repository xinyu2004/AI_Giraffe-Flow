#!/usr/bin/env bash
# TEMPLATE: copy to SKU scripts/_common.sh then fork.
# Shared helpers for project compile/run (host or cross).
...
# Fail fast if generated/ is missing — do not run python -m gf_codegen.compose here.
...
gf_sil_need_stage() {
  ...
  [[ -e "${rt}/lib/libgf_channel.so" || -e "${rt}/lib/libgf_channel.so.0" || -e "${rt}/lib/libgf_channel.so" || -e "${rt}/lib/libgf_channel.so.0" ]] || return 0
