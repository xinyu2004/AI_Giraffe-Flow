...
function w = gf_plan_obj_weight(lat, heading, is_ped, len_m, cls, c0)
  p = gf_plan_cal();
  ...
  % Cut-in only if already near the host band (not 10 m off-lane CIPV).
  ...
  if gap < p.cutin_approach_m && lat * heading < -0.02
